    ...
    <table class="table table-striped table-hover" id="datatable-excel">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $pcs = $item->lot_activities()->whereDate('created_at','>=',($request->from_date ?? date('Y-m-d', strtotime('-1 month'))))->whereDate('created_at','<=',($request->to_date ?? date('Y-m-d')))->sum('pcs') ?? 0 }}</td>
                <td>{{ $earnings = $item->lot_activities()->whereDate('created_at','>=',($request->from_date ?? date('Y-m-d', strtotime('-1 month'))))->whereDate('created_at','<=',($request->to_date ?? date('Y-m-d')))->sum('total_earning') ?? 0 }}</td>
                <td>{{ $paid_amount = $item->lot_activities()->whereDate('created_at','>=',($request->from_date ?? date('Y-m-d', strtotime('-1 month'))))->whereDate('created_at','<=',($request->to_date ?? date('Y-m-d')))->where('is_paid','1')->sum('total_earning') ?? 0 }}</td>
