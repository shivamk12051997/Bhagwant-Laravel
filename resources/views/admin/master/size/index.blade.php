            ...
            <div class="card-body">
                <form class="row align-items-center" enctype="multipart/form-data" onsubmit="event.preventDefault(); store_form(event);">
                    ...
                    <div class="col-md-10 form-group mb-3">
                        <h6>Size Name <span>*</span></h6>
                        <input type="text" class="form-control" name="name" id="" placeholder="Enter Size Name" required>
                    </div>
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    ...
                    {{-- <a href="#" class="btn btn-primary ms-auto" data-bs-toggle="modal" data-bs-target="#edit_modal" onclick="edit_modal(0)">Add Size</a> --}}
                </div>
...
@section('script')
    <script>
        ...
        function change_status(id){
            ...
            $.get('{{ url('master/size/status') }}/'+id, function(data){
                $.notify({ title:'Success', message:'Size Status Changed Successfully' }, { type:'success', });
                if(data == 1){
        ...
        function edit_modal(id){
            $('#ajax_html').html('<div class="loader-box"><div class="loader-37"></div></div>');
            $.get('{{ url('master/size/edit') }}/'+id, function(data){
                $('#ajax_html').html(data);
            });
        }
        function get_datatable(){
            $('#get_datatable').html('<div class="loader-box"><div class="loader-37"></div></div>');
            $.get('{{ route('master.size.datatable') }}', function(data){
        ...
        function store_form(event){
            ...
            $.post('{{ route('master.size.insert') }}', form_data, function(data){
                if(data != 0){
                    $.notify({ title:'Success', message:'Size Saved Successfully' }, { type:'success', });
                    get_datatable();
                    $('input.form-control').val('');
                }else{
                    $.notify({ title:'Error', message:'Size Name Has Already Been Added' }, { type:'danger', });
                }
            });
        }
    </script>
